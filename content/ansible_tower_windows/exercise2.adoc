...
To start, we will need to goto our Inventory.  So click *INVENTORY* at the top of the page, and then click on the name of our Inventory *Ansible Workshop Inventory*.
Now that you are on the Inventory Details page, we will need to go select our Host.  So click *HOSTS*.

Next to each host is a checkbox.  Check the box next to each host you want to run an Ad-Hoc Command on.  You will then see the image:2-adhoc-run-command-button.png[Run Command,20,20] button become enabled.  Click it now.
...
image::2-adhoc-run-win_ping.png[title="Run Win_Ping",width=1000]
...
image::2-adhoc-run-win_ping-log-1.png[title="Win_Ping Log Details",width=1000]
...
image::2-adhoc-run-win_ping-log-output.png[title="Win_Ping Log Details",width=1000]

The results returned will be differet depending on which module is used, as they all handle and deal with different data sets depending on the task.
...
|MACHINE CREDENTIAL|Student Account
|===
Launch that and view the results.  You will see that it returns a direct output of what the Powershell command returned.
...
Lets go ahead and run the `setup` module to look at the output.  Fill out the *EXECUTE COMMANDS* form again with this info.
...
image::2-adhoc-run-setup-output.png[title="Setup Log Details",width=1000]
...
image::2-adhoc-run-win_feature-output.png[title="Win_Feature Log Details",width=1000]
...
|===
|MODULE |win_service
|ARGUMENTS|name=name=W3Svc state=started
|MACHINE CREDENTIAL|Student Account
|===
...
|===
|MODULE |win_service
|ARGUMENTS|name=name=W3Svc state=stopped
|MACHINE CREDENTIAL|Student Account
|===
